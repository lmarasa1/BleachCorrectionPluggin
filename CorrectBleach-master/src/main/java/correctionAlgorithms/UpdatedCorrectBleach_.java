...
public class UpdatedCorrectBleach_ implements PlugInFilter {
		ImagePlus imp;

		String[] CorrectionMethods =  { "Simple Ratio", "Exponential Fit", "Histogram Matching", "Double Exponential Function" };
		...
		*/
		...
		public void run(ImageProcessor ip) {
			...
			else if (CorrectionMethod ==3) { //Double Exponential Function
				BleachCorrection_DoubleExpoFit BCEF;
				if (curROI == null) {
					BCEF = new BleachCorrection_DoubleExpoFit(impdup);
				} else {
					BCEF = new BleachCorrection_DoubleExpoFit(impdup, curROI);
				}
		...
		public boolean showDialog()	{
			...
			if (gd.wasCanceled())
				return false;
			UpdatedCorrectBleach_.setCorrectionMethod(gd.getNextChoiceIndex());
			return true;
