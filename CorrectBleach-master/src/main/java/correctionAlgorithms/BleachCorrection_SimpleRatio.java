...
public class BleachCorrection_SimpleRatio {
	...
	public BleachCorrection_SimpleRatio(ImagePlus imp, double baselineInt) {
       ...
       BleachCorrection_ExpoFit dFit = new BleachCorrection_ExpoFit(impdup);
//       BleachCorrection_ExpoFit dFit3D = new BleachCorrection_ExpoFit(impdup);
       
      
		
		
		// rb = ratio background
		  int[] impdimBck = imp.getDimensions();
		  int zframes = impdimBck[3];
		  int tframes = impdimBck[4];
		  if (impdimBck[3]>1 && impdimBck[4]>1){	// if slices and frames are both more than 1
				is3DT =true;
		  }
		  
		  CurveFitter cf;
		  if (is3DT) cf = dFit.decayFitting3D(zframes, tframes);
		  else cf =   dFit.dcayFitting();
		  double[] backgroundParam = cf.getParams();
		   baselineInt = backgroundParam[2];
		  //return baselineInt;
